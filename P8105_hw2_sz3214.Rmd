...
Data cleaning process: Importing 4 tables, seperating `baker_name` in table bakers, and making `baker` in other tables have the same variable name with bakers which is `first_name`.
Rearrange the viewers table, making the variables match other tables and deleting the redundant part of the `series` values, then change the variable type towards integer.
...
After merging 4 tables, I find the sort of columns were a little bit mess, so I rearrange the sort of columns.
