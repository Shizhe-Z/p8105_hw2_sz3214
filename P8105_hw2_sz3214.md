...
Data cleaning process: Importing 4 tables, seperating `baker_name` in
table bakers, and making `baker` in other tables have the same variable
name with bakers which is `first_name`. Rearrange the viewers table,
making the variables match other tables and deleting the redundant part
of the `series` values, then change the variable type towards integer.
When checking the tables, I find there is no data about series 9 and 10
in table bakes. Also Joanne’ name was recorded as Jo. So I changed
Joanne’s name. After merging 4 tables, I find the sort of columns were a
little bit mess, so I rearrange the sort of columns. And sort the data
by series and episode. In final gbb dataset, there are all variables
about bakes and bakers in series 1-8, and their relevant. There are 12
variables and 1129 observations in it.
